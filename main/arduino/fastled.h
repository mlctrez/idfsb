...
#include "../FastLED/src/lib8tion/scale8.h"
...
static uint8_t led_strip_pixels[STRIP_LED_COUNT * 3];

rmt_channel_handle_t led_chan = NULL;
rmt_encoder_handle_t led_encoder = NULL;
rmt_transmit_config_t tx_config = {.loop_count = 0};

int fastLedNumLeds = 0;
CRGB *ledData;
uint8_t brightness = 0;

class FastLEDMock {
public:
  void addLeds(gpio_num_t dataPin, struct CRGB *data, int numberOfLeds) {

    fastLedNumLeds = numberOfLeds;
    ledData = data;
  ...
  void setMaxPowerInVoltsAndMilliamps(float volts, long milliamps) {
    // TODO: determine what this does in fastled
  }

  void setBrightness(uint8_t brightnessIn) {
    brightness = brightnessIn;
  }

  void show() {
    for (int i = 0; i < fastLedNumLeds; ++i) {
      CRGB &crgb = ledData[i];
      crgb.fadeToBlackBy(255-brightness);
      led_strip_pixels[i * 3 + 0] = crgb.g;
      led_strip_pixels[i * 3 + 1] = crgb.r;
      led_strip_pixels[i * 3 + 2] = crgb.b;
    }
