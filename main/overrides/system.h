...
void check_current_function(){
  // TODO: update sensory bridge code as well to remove warning
...
void init_system(){
  ...
  pinMode(PHOTONS_PIN, ANALOG);
  pinMode(CHROMA_PIN, ANALOG);
  pinMode(MOOD_PIN, ANALOG);

  pinMode(RNG_SEED_PIN, ANALOG);

  init_serial(500000);

  FastLED.addLeds(LED_DATA_PIN, leds_out, STRIP_LED_COUNT);  // GRB ordering is assumed
