...
esp_err_t init_i2s() {
  ...
  ESP_RETURN_ON_ERROR(result, "init_i2s", "failure in i2s_driver_install");

  // ESP32-S2 changes to help SPH0645 mic
  REG_SET_FIELD(I2S_TIMING_REG(I2S_PORT), I2S_RX_SD_IN_DELAY, 2);
  REG_SET_BIT(I2S_CONF_REG(I2S_PORT), I2S_RX_MSB_SHIFT);

  result = i2s_set_pin(I2S_PORT, &pin_config);
  ESP_RETURN_ON_ERROR(result, "init_i2s", "failure in i2s_set_pin");

  result = i2s_start(I2S_PORT);
  ESP_RETURN_ON_ERROR(result, "init_i2s", "failure in i2s_start");

  ESP_LOGI("i2s.h", "I2S driver installed.");
